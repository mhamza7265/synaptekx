@extends('admin.layouts.main')
    ...
    @section('content')
        <div class="page-meta d-flex justify-content-between align-items-center mb-2">
            ...
            <a href="{{route('admin.blogs.create')}}" class="btn btn-md btn-success">Create New</a>
        </div>
                        ...
                        <table id="style-1" class="table style-1 dt-table-hover non-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="created-at">{{$blog->created_at->format('d-m-Y')}}</td>
                                    <td class="updated-at">{{$blog->updated_at->format('d-m-Y')}}</td>
                                    <td class="text-center">
                                        <a class="btn btn-sm btn-success" href="{{route('blogs-details', ['slug' => $blog->slug])}}" target="_blank">
                                            <i class="fa fa-eye"></i>
                                        </a>
                                        <a class="btn btn-sm btn-warning" href="{{route('admin.blogs.edit', $blog->slug)}}">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                        <form action="{{ route('admin.blogs.destroy', $blog->id) }}" method="POST" style="display: inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="delete-blog-btn btn btn-sm btn-danger">
