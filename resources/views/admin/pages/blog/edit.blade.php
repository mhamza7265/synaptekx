            ...
            <div class="col-lg-12">
                <div class="statbox widget box box-shadow">
                    <div class="widget-content widget-content-area">
                        <div class="card">
                            <h4 class="text-center mt-3">Edit Blog Post</h4>
                                ...
                                <form action="{{ route('admin.blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="d-flex justify-content-between align-items-baseline">
                                        ...
                                        <div class="form-group" style="width: 49%">
                                            ...
                                            <input type="text" name="slug" placeholder="blog slug" value="{{old('slug', $blog->slug)}}" class="form-control mt-2" id="slug" readonly>
                                            @error('slug')
                                                <div class="text-danger">{{ $message }}</div>
                                            @enderror
                                        </div>
                                    ...
                                    <textarea class="form-control mt-2" name="meta_description" id="meta_description" cols="30" rows="5">{{old('meta_description', $blog->meta_description)}}</textarea>
                                    @error('meta_description')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror

                                    <span class="d-block mt-3">Keywords/Tags:</span>
                                    <textarea class="form-control mt-2" name="keywords" id="keywords" cols="30" rows="5">{{old('keywords', $blog->keywords)}}</textarea>
                                    @error('keywords')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                    ...
                                    <textarea name="blog_body" class="form-control summernote mt-2">{{old('blog_body', $blog->blog_body)}}</textarea>
                                    @error('blog_body')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror

                                    <button class="btn btn-md btn-success mt-3">Update</button>
                                </form>
                            </div>
                        </div>
                    </div>
